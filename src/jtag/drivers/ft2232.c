...
/********************************************************************
 * Support for Advanced Communication H-Type JTAG adapter (ACHJTAG).
...
static int achjtag_init(void)
{
	 ...
	 * and should use this "achjtag" layout.
	 *
...
static int achjtag_deinit(void)
{
	...
	low_output	|= ACHI_DBG_ENn;
	if (ft2232_set_data_bits_low_byte(low_output, low_direction) != ERROR_OK) {
		LOG_ERROR("couldn't deinitialize FT2232 low-byte with 'achjtag' layout");
		return ERROR_JTAG_DEVICE_ERROR;
	}
	...
	high_direction	= 0x00;
	if (ft2232_set_data_bits_high_byte(high_output, high_direction) != ERROR_OK) {
		LOG_ERROR("couldn't deinitialize FT2232 high-byte with 'achjtag' layout");
		return ERROR_JTAG_DEVICE_ERROR;
	}
...
{
	/*
	 * ACHJTAG has two LEDs connected to ACBUS3 and ACBUS4,
